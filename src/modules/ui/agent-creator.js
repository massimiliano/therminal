...
"""`,
  ...
  claude: {
    ...
    prompt: `Sei un Lead Software Architect esperto in automazione dello sviluppo e sistemi Multi-Agente.
Il tuo compito è analizzare la codebase del progetto corrente e generare una suite completa di subagenti personalizzati per Claude Code.

Per farlo correttamente, devi aderire rigorosamente a questo schema tecnico per la creazione dei file.

SCHEMA DEI SUBAGENTI CLAUDE CODE:
Ogni agente deve essere definito in un file Markdown separato all'interno della cartella .claude/agents/.
Il file deve iniziare con un blocco frontmatter YAML (delimitato da ---), seguito dal corpo del testo che funge da system prompt.

Campi frontmatter supportati:
- name (REQUIRED): Nome identificativo in kebab-case (es. db-guru, electron-shield).
- description (REQUIRED): Spiegazione chiara di quando Claude dovrebbe delegare i task a questo subagente.
- model (OPTIONAL): Usa sonnet per ragionamenti complessi (architettura, scrittura) o haiku per ricerca/mappatura veloce.
- tools (OPTIONAL): Lista dei tool permessi (es. Read, Grep, Glob, Bash, Edit, Write, Agent). Se omesso, eredita tutti i tool.
- disallowedTools (OPTIONAL): Strumenti da negare esplicitamente.
- memory (OPTIONAL): Imposta su project per abilitare la memoria persistente e far ricordare all'agente le decisioni architetturali tra le varie sessioni.
- permissionMode (OPTIONAL): default, acceptEdits, dontAsk, bypassPermissions, o plan.

REGOLE DI ARCHITETTURA DEGLI AGENTI:
1. Analizza l'intera struttura delle directory e individua i domini logici principali (es. strati architetturali, moduli core, UI, state management).
2. Genera un agente specializzato per ogni dominio critico per evitare "context pollution". Non hai limiti al numero di agenti.
3. Il corpo del file Markdown deve contenere istruzioni direttive e iper-focalizzate, citando i file o le cartelle reali del progetto di cui l'agente è responsabile.
4. Per agenti di indagine/ispezione: limita i tool (es. tools: Read, Grep, Glob, Bash) e usa model: haiku.
5. Per agenti di implementazione/refactoring: consenti tool di scrittura, usa model: sonnet e abilita memory: project.
...
Sei l'architetto del routing di questo progetto. Analizza i file in /src/routes e verifica che i middleware di autenticazione siano applicati correttamente. Aggiorna la tua memoria con le rotte scoperte.

Inizia l'analisi della codebase e genera la suite completa di subagenti.`,
  },
};
